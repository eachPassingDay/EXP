...
if __name__ == '__main__':
    ...
    if args.id >= 0:
        # 使用 args.id 来选择跑哪个序列
        target_scenes = [ bonn_split[args.id] ]
    else:
        target_scenes = bonn_split

    ate_list = []
    for scene in bonn_split:
        print("Performing evaluation on {}".format(scene))
        torch.cuda.empty_cache()

        args.config = "configs/BONN/bonn.yaml"
        ...
        if 'data' not in cfg:
            cfg['data'] = {}
        # 动态给每个序列分配独立的输出文件夹，完美替代那一堆单独的 yaml 文件
        cfg['data']['output'] = "output/BONN/" + scene
