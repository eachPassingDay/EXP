...
class gs_tracking_mapping():
    ...
    def mapping(self, idx, gt_color, gt_depth, gt_c2w, estimated_c2w, seg_mask, init = False):        
        ...
        if (idx % self.keyframe_every == 0 or (idx == self.n_img-2)) and (idx not in self.keyframe_list) and (not torch.isinf(gt_c2w).any()) and (not torch.isnan(gt_c2w).any()):
            ...
            # --- 插入的渲染与保存图像代码开始 ---
            with torch.no_grad():
                # 1. 在 output 目录下创建一个 render_keyframes 文件夹用于保存渲染图
                render_dir = os.path.join(self.output, "render_keyframes")
                ...
                # 4. 调用原代码中现成的 render 包进行渲染
                render_pkg = render(
                    self.gaussians_xyz, self.gaussians_features_dc, self.gaussians_features_rest,
                    gaussians_opacity_activation, gaussians_scaling_activation, gaussians_rotation_activation,
                    self.gaussians.get_active_sh_degree(), self.gaussians.get_max_sh_degree(),
                    camera_center, world_view_transform, self.projection_matrix,
                    self.fovx, self.fovy, self.H, self.W
                ...
                # 5. 提取出渲染好的图片张量，将其维度从 [C, H, W] 转换到 OpenCV 需要的 [H, W, C]
                image_render = render_pkg["render"].permute(1, 2, 0).detach().cpu().numpy()
                
                # 6. 将数据范围从 [0, 1] 映射到 [0, 255]，并把 RGB 格式转为 BGR 格式
                image_render_bgr = cv2.cvtColor((np.clip(image_render, 0.0, 1.0) * 255).astype(np.uint8), cv2.COLOR_RGB2BGR)
                
                # 7. 保存图片
                save_path = os.path.join(render_dir, f"frame_{idx.item():05d}.png")
                cv2.imwrite(save_path, image_render_bgr)
            # --- 插入的渲染与保存图像代码结束 ---
